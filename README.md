...
このアプリの履歴はサーバー側に保存され、TTL 経過後に自動で破棄されます。「新しいチャット」は会話の破棄ではなくセッションの切り替えとして動作し、前の会話は左側の履歴一覧に残ります。授業中にページを更新しても会話を戻せますが、共有端末での残りっぱなしを避けるため、保存から一定時間が経つとサーバー側から自動削除されます。
