...
履歴保存について:
- チャット履歴はブラウザ内に一時保存されます
- 「新しいチャット」を押すと、今の会話は消えずに下の履歴へ退避されます
- 授業中の再読み込みでは履歴が復元されます
- 一定時間で自動削除されます（既定は 30 分）。サーバー側環境変数 `CHAT_HISTORY_TTL_MINUTES` で調整できます。
- 共有端末では履歴を使い終わったあとに手動で整理できます
...
このアプリの履歴はサーバーには保存せず、ブラウザの一時保存のみを使います。「新しいチャット」は会話の破棄ではなくセッションの切り替えとして動作し、前の会話は左側の履歴一覧に残ります。授業中にページを更新しても会話を戻せますが、共有端末での残りっぱなしを避けるため、保存から一定時間が経つと自動で破棄されます。
